...
import com.zexceed.skripsiehapp.ui.adapter.HomePagerAdapter
import com.zexceed.skripsiehapp.ui.viewmodel.AuthViewModel
import com.zexceed.skripsiehapp.util.HomeTabs
import com.zexceed.skripsiehapp.databinding.FragmentHomeBinding
...
class HomeFragment : Fragment() {
    ...
    private fun viewPager2SetupWithTabLayout(tabLayout: TabLayout, viewPager2: ViewPager2){
        TabLayoutMediator(tabLayout, viewPager2) { tab, position ->
            ...
            when (position) {
                HomeTabs.PEMINJAMAN.index -> {
                    tab.customView = view
                    textViewTitle.text = getString(R.string.peminjaman)
                    tab.onSelection(true)
                }
                HomeTabs.INVENTORY.index -> {
                    tab.customView = view
                    textViewTitle.text = getString(R.string.inventory)
    ...
    textViewTitle?.let { textView ->
        textView.setTextColor(
            ContextCompat.getColor(
                textView.context,
                if (isSelected) R.color.font_color_dark else R.color.font_color_grey
            )
        )
    }
}